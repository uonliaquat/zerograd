...
static inline void data_init(Tensor *tensor, void *data){
    // Initializing data
    tensor->data = (void*)calloc(tensor->size,  tensor->elem_size/8);
    if(data && tensor->data) memcpy(tensor->data, data, tensor->size *  tensor->elem_size/8);
